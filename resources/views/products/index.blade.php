...
@section('content')
    <div class="container mx-auto px-4">
        ...
        {{-- //table for product --}}
        <table
            class="w-full mt-5 text-center text-sm sm:text-base lg:text-lg border-collapse bg-white shadow-md rounded-lg overflow-hidden">
            <thead>
            ...
            <tbody class="text-gray-600">
                @foreach ($products as $product)
                    <tr class="hover:bg-gray-100 transition-colors duration-300">
                        ...
                        <td class="border px-4 py-3">
                            <span
                                class="inline-block px-3 py-1 text-sm font-semibold {{ $product->status == 'Show' ? 'bg-green-200 text-green-700' : 'bg-red-200 text-red-700' }} rounded-full">
                                {{ $product->status }}
                            </span>
                        </td>
